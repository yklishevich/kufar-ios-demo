...
sync_repo() {
  ...
  # Без ветки слежения вопрос «сколько коммитов не отправлено» не имеет
  # ответа, а `log @{u}..` на это отвечает пустым выводом — неотличимо
  # от «всё отправлено». Раньше здесь стоял именно он, и после сброса
  # истории (upstream пропадает у всех семи репозиториев разом) скрипт
  # уверенно рапортовал, что делать нечего, при неотправленной работе.
  #
  # «?» вместо числа: неизвестность не притворяется нулём, а до пуша
  # дело всё равно дойдёт — ниже проверяется неравенство нулю.
  if git -C "$dir" rev-parse --abbrev-ref '@{u}' >/dev/null 2>&1; then
    ahead="$(git -C "$dir" log --oneline @{u}.. | wc -l | tr -d ' ')"
  else
    ahead="?"
  fi
