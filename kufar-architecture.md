...
Демо-реализация: [kufar-ios](./README.md).
...
| **Features** | `Search`, `Posting`, `Goods`, `Auto`, `RealEstate`, `Travel`, `Profile`, `Auth` | Есть домен, маршруты и экраны |
| **Platform** | `Navigation`, `DesignTokens`, `DesignComponents`, `ListingKit`, `SchemaKit`, `AnalyticsImpl`, `FeatureFlags` | Есть SwiftUI или побочные эффекты, но нет домена |
| **Контракты** | `CatalogContracts`, `*Interface`, `SessionInterface`, `AnalyticsAPI` | Только типы и протоколы, ни одной реализации |
| **Foundation** | `SharedKernel`, `Networking`, `Persistence` | Чистый Swift, ни строчки SwiftUI |

Единого `Core` нет намеренно: у сети, аналитики и дизайн-системы разный темп изменений и разные владельцы. Один `Core` означает, что правка отступа в кнопке инвалидирует кеш сборки всего приложения.
...
**`ListingKit` — не второй `Core`,** и на этом стоит остановиться самому, не дожидаясь вопроса. Это узкий модуль с одной ответственностью: каркас карточки объявления. Он не знает ни одной вертикали, и линтер это проверяет. Подробно — раздел 5.
...
**Критерий проверки всей схемы:** каждая вертикаль собирается и запускается в отдельном демо-приложении без остальных. Не запускается — протекла горизонтальная зависимость.
...
Имя репозитория не значит ничего — `bootstrap.sh` клонирует `KufarFoundation.git` в папку `kufar.Foundation`. Отступишь от формата, и Xcode откажется подменять:
...
Правило найдено экспериментом, и путь к нему стоит рассказать, потому что он показывает цену неполной проверки. Сначала CLI ответил категорично:
...
Выигрыш конкретный. Границы владения совпали с границами репозиториев, поэтому `CODEOWNERS` стал одной строкой. Правка контракта вместе с реализацией — `kufar.GoodsContracts` и `kufar.Goods` лежат рядом — теперь **один коммит вместо двух PR с тегом между ними**, а таких изменений на порядок больше, чем кросс-командных. Наборов CI стало шесть вместо двадцати одного.
...
**Цена переезда, названная честно.** История сохранена через `git subtree`, но SHA изменились — ссылки на коммиты в старых задачах протухли. PR, ревью и обсуждения `subtree` не переносит вовсе: они остались в старых репозиториях, поэтому те **архивированы, а не удалены**. Это стоит ноль и спасает половину контекста.
...
graph TD
    ...
    API["<b>AnalyticsAPI</b><br/>protocol AnalyticsTracking<br/>struct Event<br/><i>зависимостей нет вообще</i>"]

    Impl["<b>AnalyticsImpl</b><br/>AmplitudeTracker<br/>очередь · батчинг · персистентность<br/><i>import Amplitude, Networking</i>"]
    Spy["<b>SpyTracker</b><br/>копит события в массив"]
    ...
    Spy -.->|conforms| API
    CR -->|import| Impl
    CR -->|"inject any AnalyticsTracking"| RE
    CR -->|inject| Goods
...
Вендорский SDK не попадает в граф ни одной вертикали. Замена Amplitude переписывает один класс, сигнатуры проверит компилятор.
...
graph TD
    subgraph feat["Features"]
        Auth["<b>Auth</b><br/>AuthInterceptor<br/>Bearer-токен, обработка 401, refresh"]
    end

    subgraph found["Foundation"]
        Net["<b>Networking</b><br/>объявляет protocol RequestInterceptor<br/>про токены не знает ничего"]
    end

    Auth ==>|"зависимость сборки — вниз"| Net
...
`Vertical` — единственное место, где поломка сборки задумана. Добавили `realEstate` — компилятор останавливает пять точек у трёх команд, и в каждой нужно продуктовое решение: какой маршрут пушить из ленты, как рисовать сохранённую карточку в избранном, куда вести после публикации, что делать с диплинком без категории. `default` превратил бы это в тихое «тап ничего не делает», причём только для новой вертикали — то есть там, где ещё никто не тестирует. Компилятор выдаёт исчерпывающий чек-лист; человек с грепом составит его с пропусками, и первым выпадет `FavoritesScreen`.

Цену стоит назвать самому: `Vertical` живёт в `SharedKernel`, значит новая вертикаль — это согласованные PR в три чужих репозитория. Дорого. Но эти PR понадобятся в любом случае, вопрос только в том, узнаешь ты о них на сборке или из отзывов в App Store. И вертикали добавляют раз в год, а не раз в спринт.
...
graph TD
    ...
    Comp["<b>AppComposition</b><br/>GoodsDestinations(repo:)<br/>.concat(ProfileDestinations(repo:))<br/>.concat(AuthDestinations(session:))"]
...
**Что говорить.** `Goods` знает, *куда* идти, но не знает, *что* нарисуется. `NavigationPath` принимает любой `Hashable`, поэтому `Router` тоже не знает ни одного конкретного маршрута — он общий примитив на 15 строк.
...
graph TD
    ...
    Push["Пуш: новое сообщение"] -->|"open(route, in: .messages)"| AppRouter
...
- **Вкладки не отражают вертикали.** Табы — это режимы работы пользователя: искать, вернуться к отложенному, вести переписку, управлять своим. Вертикаль живёт внутри поиска как категория фильтра (4.1). Сделать вкладку на вертикаль — значит привязать корневой экран к каталогу и получать PR в репозиторий команды приложения на каждое новое направление.
...
*Ровно одно место в приложении знает про все вертикали сразу.* Это лента:
...
**Ожидаемый вопрос: «а кому тогда принадлежит лента?»** Отдельной команде поиска. Это честный ответ и на организационный вопрос тоже: лента, фильтры, избранное и сохранённые поиски — общие поверхности, у них свои метрики и свой владелец. Вертикали владеют карточкой и доменом.
...
Что из этого стоит проговорить:

**Точек схождения две, и обе не принадлежат вертикалям.** Пока такое место одно, оно выглядит случайностью; когда их две и устроены они одинаково — виден приём. Добавится вертикаль — компилятор приведёт ровно в два `switch`, и оба в модулях, которые и так знают про все вертикали.
...
graph LR
    ...
    Kind["<b>enum FieldKind</b><br/>select · range · toggle · справочник<br/><b>.unknown</b> ← forward compatibility"]
...
Этот же движок (`SchemaKit` внутри `ListingKit`) рендерит блок атрибутов в карточке объявления — см. 5.1. Одна схема описывает и «по чему ищем», и «что показываем».
...
**Что говорить.** Бесконечный скролл с картинками — главный экран и главный источник жалоб на память. Ключевое: downsampling до размера ячейки, а не хранение полноразмерного изображения; отмена загрузки, когда ячейка ушла с экрана; бюджет на hitch rate, зафиксированный числом, а не ощущением.
...
**Связка с продуктом — сильный ход.** Технические метрики соединяются с продуктовыми в одну фразу: сократили TTI ленты → выросла глубина просмотра → выросло число контактов продавцам → выросла ликвидность, главная метрика классифайда.
...
graph TD
    subgraph kit["ListingKit · Platform · ничего не знает о вертикалях"]
        ...
        Schema["<b>SchemaKit</b><br/>рендерер декларативных схем<br/><i>тот же, что у фильтров</i>"]
    end
...
**Что проверяет линтер:** `import *Interface` любой вертикали внутри `ListingKit`. Как только каркас узнаёт про недвижимость — он перестал быть каркасом.
...
**Где приём кончается** — сказать самому, не дожидаясь вопроса. Если сервер решает не только содержимое блоков, но и их состав и порядок, слотов фиксированной арности не хватает. Ответ тот же, что для фильтров: это всё равно каталог, значит `enum BlockKind` + `switch` внутри конкретной `BlockRow`, а не `[AnyView]`.
...
Это тот самый вопрос из сопроводительного — как строить чистую архитектуру в SwiftUI-проекте. Архитектурные границы здесь упираются в устройство рендера, и это стоит показать явно.
...
**Инструмент для разговора:** `let _ = Self._printChanges()` в `body`. `@self changed` — вью пересоздана; имя конкретного свойства — обновление на месте. Плюс Instruments → SwiftUI → View Body: счётчик растёт быстрее числа появившихся ячеек — дерево перестраивается там, где могло обновиться.

---

## 7. «Чистая» ли это архитектура

В сопроводительном стоит слово «чистая», поэтому вопрос прозвучит почти наверняка — и отвечать на него лучше первым, а не оправдываться потом.

> Это **не** Clean Architecture по форме. Это модульная архитектура с соблюдённым Dependency Rule.

Формулировка снимает половину вопросов сразу: слушатель перестаёт искать интеракторы и презентеры, которых здесь нет и не должно быть.
...
| Механизм | Что ловит |
|---|---|
| Линтер зависимостей в CI | Импорт вертикали из вертикали, импорт реализации вместо контракта, `import SwiftUI` в `*Interface`, `import *Interface` вертикали внутри `ListingKit`, папку не в формате `scope.Name` |
...
| Сборка воркспейса на HEAD всех репозиториев | Рассогласование незарелиженной работы между пакетами |
...
Про CI стоит сказать отдельно, потому что здесь легко показать понимание multi-repo — и легко провалиться на вопросе «а как вы это собираете».

Локально пакеты подключены **локальной подменой**: Xcode сопоставляет зависимость с папкой в воркспейсе по identity и подставляет её вместо загрузки из реестра. Идентичность для реестра — это `scope.Name`, а у локального пакета она берётся из имени директории, отсюда правило «папка называется `kufar.Foundation`». Версии из манифестов при этом не проверяются вообще — ты всегда собираешься против HEAD соседа. Это семантика монорепозитория: правка в `SharedKernel` видна восьми потребителям сразу, кросс-пакетное изменение делается одним движением.
...
| Когда | на каждый PR | ночью и по кнопке |
...
Ни один режим не подменяет другой. Локальный никогда не покажет конфликт версий — он их не смотрит. Версионный никогда не покажет, что твоя правка ломает соседа, пока ты не выпустишь тег.
...
**Чего не хватает для настоящего CI** — сказать вслух, если спросят. Shared-схема у приложения есть и закоммичена, но её `TestAction` пуст: тестовые таргеты пакетов (`ArchitectureTests`, `AppFeatureTests`) к схеме не подключены, и `xcodebuild test` прогонит ноль тестов, отчитавшись зелёным. Чинится одним диалогом Edit Scheme, но показательно само место поломки: тесты лежат в пакетах, схема живёт в проекте приложения, владельцы у них разные, а связь между ними не проверяет никто. Зелёный CI, не запускающий ни одного теста, — самый дорогой вид ложной уверенности. И проверки «поднял ли пол в манифесте, когда начал использовать новое API» нет ни одной — она ловится только косвенно, красным `pinned`.

И про миграцию, если спросят: модуляризация монолита делается инкрементально, по модулю за раз. Умение рассказать постепенную стратегию ценится выше идеальной целевой схемы.
