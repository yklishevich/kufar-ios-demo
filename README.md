...
Шесть репозиториев по числу команд. Границы владения совпали с границами репозиториев: `CODEOWNERS` стал одной строкой, а правка контракта вместе с реализацией — одним коммитом вместо двух PR. Версии при этом независимы, теги несут имя пакета: `kufar.Foundation-1.2.0`.
...
В каждом репозитории лежит `CODEOWNERS` — так владение выражают в жизни, а не в договорённостях: GitHub по этому файлу автоматически назначает ревьюеров и, при включённой защите ветки, не даёт смержить PR без апрува владельца.
...
Синтаксис — `<шаблон пути> <владелец> [<владелец> ...]`. Раз репозиторий теперь ровно один пакет, правило в каждом одно: `*  @kufar/ios-goods @kufar/ios-platform` для контрактных, `*  @kufar/ios-goods` для остальных.
...
Побочная выгода от «репозиторий = пакет»: `CODEOWNERS` перестал быть лабиринтом путей. Границы владения теперь совпадают с границами репозиториев, и файл описывает их одной строкой.
