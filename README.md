...
| **Identity** | `kufar.IdentityContracts`, `kufar.Identity` | `SessionInterface`, `ProfileInterface`, `AuthInterface`, `Profile`, `Auth` |
...
### Почему команд шесть, а пакетов двадцать
...
У платформенной команды пакетов десять — токены генерируются из Figma, каркас карточки меняется реже вертикалей, приложение собирается последним. Разные релизные циклы, одни люди.
